...
When you look at the number we started to and the number we ended with they vary drastically. When you change the sample size you can see that the numbers change based off sample size and every time you knit the assignment. This happens because we keep changing the sample size to get smaller but its still picking random number to fit in the sample size based off the original data. We see that there is not always 1/sqrt() change in the data every time we change the sample size.